...
Scenario Outline:  User successfully removes an ingredient to the recipe
    ...
    | Fireball | Canadian whisky,sweetener,cinnamon fjavouring | sweetener     | Canadian whisky,cinnamon flavouring | UPDATE-RECIPE-REMOVE-INGREDIENT| 
...
Scenario: Admin deletes a drink recipe
	When the user "admin" with password "adpassword1" is logged into their account
   	And the admin "admin" deletes the drink "Fireball" by "User1"
...
Scenario Outline: Admin successfully changes status of the recipe
	...
	When the admin "admin" with password "adpassword1" is logged into their account
	And the admin changes the "<drink>"'s status by "<author>" to "<status>"
	Then a confirmation message "<confirmation>" shall be raised

	Examples:
    | drink     | author | status | confirmation         | 
    | Fireball   | User1 | private| UPDATE-RECIPE-STATUS |
    | Mint Julep | User1 | public | UPDATE-RECIPE-STATUS |
...
Scenario Outline: Admin successfully adds an ingredient to the recipe
	...
	When the admin "admin" with password "adpassword1" is logged into their account
	And the admin modifies the drink "<name>" by "<author>" by adding a new ingredient "<newingredient>"
	Then the new ingredient "<newingredient>" shall be added to drink "<name>"
	Then a confirmation message "<confirmation>" shall be raised
	
	Examples:
    | name     | author | ingredients                                   | newingredient | confirmation       |
    | Fireball | User1 | Canadian whisky,sweetener,cinnamon flavouring | lime          | UPDATE-RECIPE-INGREDIENT| 
    | Mojitos  | User1 | white rum, sugar,lime juice,soda water,mint   | lime	       | UPDATE-RECIPE-INGREDIENT|
...
Scenario Outline: Admin adds duplicate ingredient to the recipe
  ...
  Examples:
    | name    | author | ingredients                                 | newingredient | newingredientlist                                | error                   |
    | Mojitos | User1 | white rum, sugar,lime juice,soda water,mint | mint          | white rum, sugar,lime juice,soda water,mint,lime | UPDATE-RECIPE-DUPLICATE |
...
Scenario Outline:  Admin successfully removes an ingredient to the recipe
	...
	The ingredients list is updated
  	...
  	When the admin "admin" with password "adpassword1" is logged into their account
	And the admin modifies the drink "<name>" by "<author>" by removing the ingredient "<oldingredient>"
	Then the ingredient "<oldingredient>" shall be removed from the drink "<name>"
	Then a confirmation message "<confirmation>" shall be raised

	Examples:
    | name     | author | ingredients                                   | oldingredient | newingredientlist                   | confirmation       |
    | Fireball | User1 | Canadian whisky,sweetener,cinnamon flavouring | sweetener     | Canadian whisky,cinnamon flavouring | UPDATE-RECIPE-REMOVE-INGREDIENT| 
    | Mojitos  | User1 | white rum, sugar,lime juice,soda water,mint   | lime juice    | white rum, sugar,soda water,mint    | UPDATE-RECIPE-REMOVE-INGREDIENT|
